...
void SSSession::RecvEncryptSend(asio::ip::tcp::socket &sock1,
    ...
    sock1.async_receive(asio::buffer(buff),
                        [self, this, &sock1, &sock2, &buff](const asio::error_code &ec, size_t len) {
                            ERROR_RETURN
                            Cypher::Instance().Encrypt(buff, len);
                            asio::async_write(sock2, asio::buffer(buff, len),
                                              [self, this, &sock1, &sock2, &buff](const asio::error_code &ec,
                                                                                  size_t len) {
                                                  ERROR_RETURN
...
void SSSession::LocalStart() {
    ...
    local_socket_.async_receive(asio::buffer(local_buffer_), [self, this](const asio::error_code &ec, size_t len) {
        ...
        remote_socket_.async_connect(SSConfig::Instance().ServerEndpoint(), [self, this](const asio::error_code &ec) {
            ...
            asio::async_write(remote_socket_, asio::buffer(local_buffer_, 3),
                              [self, this](const asio::error_code &ec, size_t len) {
                                  ERROR_RETURN
                                  // send to server ok, receive from server, expected to be encrypted 05 00
                                  remote_socket_.async_receive(asio::buffer(remote_buffer_),
                                                               [self, this](const asio::error_code &ec, size_t len) {
                                                                   ERROR_RETURN
                                                                   if (len < 2) {
                                                                       log("fd=%d, %s", local_socket_.native_handle(),
                                                                           "receive 05 00 from remote err");
                                                                       return;
                                                                   }

                                                                   Cypher::Instance().Decrypt(remote_buffer_, 2);
                                                                   if (remote_buffer_[0] != 0x05 ||
                                                                       remote_buffer_[1] != 0x00) {
                                                                       log("fd=%d, %s", local_socket_.native_handle(),
                                                                           "receive from remote, not 05 00");
                                                                       return;
                                                                   }

                                                                   // send to client
                                                                   asio::async_write(local_socket_,
                                                                                     asio::buffer(remote_buffer_, 2),
                                                                                     [self, this](
                                                                                             const asio::error_code &ec,
                                                                                             size_t len) {
                                                                                         ERROR_RETURN
                                                                                         // receive 05 01 00 01/03 ... from client
                                                                                         local_socket_.async_receive(
                                                                                                 asio::buffer(
                                                                                                         local_buffer_),
                                                                                                 [self, this](
                                                                                                         const asio::error_code &ec,
                                                                                                     ...
                                                                                                     if (len < 5) {
                                                                                                         log("fd=%d, %s",
                                                                                                             local_socket_.native_handle(),
                                                                                                             "receive 05 01 00 01/03 ... from client err");
                                                                                                         return;
                                                                                                     }

                                                                                                     // encrypt and send to remote
                                                                                                     Cypher::Instance().Encrypt(
                                                                                                             local_buffer_,
                                                                                                             len);
                                                                                                     asio::async_write(
                                                                                                             remote_socket_,
...
void SSSession::ServerStart() {
    ...
    local_socket_.async_receive(asio::buffer(local_buffer_), [self, this](const asio::error_code &ec, size_t len) {
        ...
        asio::async_write(local_socket_, asio::buffer(local_buffer_, 2),
                          [self, this](const asio::error_code &ec, size_t len) {
                              ERROR_RETURN
                              ...
                              local_socket_.async_receive(asio::buffer(local_buffer_),
                                                          [self, this](const asio::error_code &ec, size_t len) {
                                                              ...
                                                              if (len < 5) {
                                                                  log("fd=%d, %s", local_socket_.native_handle(),
                                                                      "recv encryted 05 01 00 01 ... length too short");
                                                                  local_socket_.close();
                                                                  return;
                                                              }
                                                              ...
                                                              if (local_buffer_[1] == 0x01) {
                                                                  if (local_buffer_[3] ==
                                                                      0x01) // 05 01 00 01 ip[4] port[2]
                                                                  {
                                                                      if (len != 10) {
                                                                          log("fd=%d, connect with ip length error, not 10, but %zu",
                                                                              local_socket_.native_handle(), len);
                                                                          local_socket_.close();
                                                                          return;
                                                                      }

                                                                      uint32_t int_ip = *(uint32_t *) (&local_buffer_[4]);
                                                                      dest_host_ = asio::ip::address_v4(
                                                                              ntohl(int_ip)).to_string();
                                                                  ...
                                                                  } else if (local_buffer_[3] ==
                                                                             0x03) // 05 01 00 03 host_len host[host_len] port[2]
                                                                  {
                                                                      uint8_t host_len = local_buffer_[4];
                                                                      if (static_cast<int>(len) != static_cast<int>(7 +
                                                                                                                    host_len)) // warning of comparing on gcc 4.8
                                                                      {
                                                                          log("fd=%d, connect with hostname length error, not %d, but %zu",
                                                                              local_socket_.native_handle(),
                                                                              7 + host_len, len);
                                                                          return;
                                                                      }

                                                                      dest_host_ = std::string(&local_buffer_[5],
                                                                                               host_len);
                                                                      uint16_t int_port = *(uint16_t *) (&local_buffer_[
                                                                              len - 2]);
                                                                      dest_port_ = std::to_string(ntohs(int_port));
                                                                      std::cout << "connect with host=" << dest_host_
                                                                                << ", port=" << dest_port_ << "\n";
                                                                  }

                                                                  // resolve the domain
                                                                  asio::ip::tcp::resolver::query q(dest_host_,
                                                                                                   dest_port_);
                                                                  resolver_.async_resolve(q, [self, this](
                                                                          const asio::error_code &ec,
                                                                          asio::ip::tcp::resolver::results_type results) {
                                                                      ERROR_RETURN
                                                                      // connect to dest
                                                                      const std::string host_name = (*results.begin()).host_name();
                                                                      remote_socket_.async_connect(*results.begin(),
                                                                                                   [self, this, host_name](
                                                                                                           const asio::error_code &ec) {
                                                                                                       ERROR_RETURN
