...
private:
    void HandleLocalConnect(const asio::error_code &err, std::size_t bytes_transferred) // recv 05 01 00 from client
    {
        ...
        if (local_buffer_[0] != 0x05 || local_buffer_[1] != 0x01 || local_buffer_[2] != 0x00) {
            cout << "recv 05 01 00 content error " << int(local_buffer_[0]) << ", " << int(local_buffer_[1]) << ", "
                 << int(local_buffer_[1]) << "\n";
            return;
        }
        ...
        asio::async_write(local_socket_, asio::buffer(&local_buffer_[0], 2),
                          std::bind(&Sock5Session::HandleConnectWrite,
                                    shared_from_this(), _1, _2));
    }
    ...
    void HandleConnectCmd(const asio::error_code &err, std::size_t bytes_transferred) {
        ...
        if (local_buffer_[1] == 0x01) // connect as ipv4 address
        {
            ...
            resolver_.async_resolve(q, [self, this](const asio::error_code &err,
                ...
                dest_socket_.async_connect(*results.begin(), [self, this](const asio::error_code &err) {
                    ...
                    asio::async_write(local_socket_, asio::buffer(local_buffer_, sizeof(resp)),
        ...
        local_socket_.async_read_some(asio::buffer(local_buffer_),
                                      [self, this](const asio::error_code &err, size_t bytes_transferred) {
                                          if (!err) {
                                              ...
                                              asio::async_write(dest_socket_,
                                                                asio::buffer(local_buffer_, bytes_transferred),
                                     ...
                                     [self, this](const asio::error_code &err, size_t bytes_transferred) {
                                         if (!err) {
                                             ...
                                             asio::async_write(local_socket_,
                                                               asio::buffer(dest_buffer_, bytes_transferred),
                                                               [self, this](const asio::error_code &err,
                                                                            std::size_t bytes_transferred) {
                                                                   if (!err) {
                                                                       ReadFromDest();
...
class Sock5Server {
public:
    explicit Sock5Server(asio::io_context &ioc, const asio::ip::tcp::endpoint &ep) : io_context_(ioc),
                                                                                     acceptor_(ioc, ep) {}
